...
public class UIPartyTeam : MonoBehaviour
{
    ...
    public Action<List<SO_Element>> uiUpdated;
    ...
    public void UpdateUI(int teamMemberIndex, int mythSelectionIndex, Texture teamRender)
    {
        ...
        List<SO_Element> selectedMyths = new();
        foreach (var item in selectedMythIndices)
        {
            if (item == -1) continue;
            selectedMyths.Add(mythDatas[item].myth.elementalType);
        }

        uiUpdated.Invoke(selectedMyths);
    }
}
